...
ifeq ($(WITH_DOCS),yes)
	...
	$(INSTALL) -d "${DESTDIR}/etc/mqttz"
	$(INSTALL) -m 644 mosquitto.conf "${DESTDIR}/etc/mqttz/mosquitto.conf.example"
	$(INSTALL) -m 644 aclfile.example "${DESTDIR}/etc/mqttz/aclfile.example"
	$(INSTALL) -m 644 pwfile.example "${DESTDIR}/etc/mqttz/pwfile.example"
	$(INSTALL) -m 644 pskfile.example "${DESTDIR}/etc/mqttz/pskfile.example"

uninstall :
	set -e; for d in ${DIRS}; do $(MAKE) -C $${d} uninstall; done
	rm -f "${DESTDIR}/etc/mqttz/mosquitto.conf.example"
	rm -f "${DESTDIR}/etc/mqttz/aclfile.example"
	rm -f "${DESTDIR}/etc/mqttz/pwfile.example"
	rm -f "${DESTDIR}/etc/mqttz/pskfile.example"
